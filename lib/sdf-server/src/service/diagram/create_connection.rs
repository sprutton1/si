...
pub async fn create_connection(
    ...
    ctx.write_audit_log(
        AuditLogKind::CreateConnection {
            from_component_id: request.from_component_id.into(),
            from_component_name: from_component.name(&ctx).await?,
            from_socket_id: request.from_socket_id.into(),
            from_socket_name: OutputSocket::get_by_id(&ctx, request.from_socket_id)
                .await?
                .name()
                .to_string(),
            to_component_id: request.to_component_id.into(),
            to_component_name: to_component_name.clone(),
            to_socket_id: request.to_socket_id.into(),
